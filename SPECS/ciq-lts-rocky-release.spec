...
# Cloud content directory should include leading, but not trailing, slashes so that it can be blanked out for no prefix
# at all. Cloud repos are not "gated", so they use the /public prefix.
...
Name:           %{name_prefix}rocky-release%{?rltype}
Version:        %{full_release_version}
Release:        %{rocky_rel}%{dist}
Summary:        %{distro_name} release files | CIQ LTS %{full_release_version}
...
Provides:       %{name_prefix}rocky-release = %{version}-%{release}
...
Obsoletes:      ciq-lts86-rocky-release < 10
Conflicts:      ciq-lts86-rocky-release < 10
Obsoletes:      ciq-lts88-rocky-release < 10
Conflicts:      ciq-lts88-rocky-release < 10
Obsoletes:      ciq-lts92-rocky-release < 10
Conflicts:      ciq-lts92-rocky-release < 10
Obsoletes:      ciq-lts94-rocky-release < 10
Conflicts:      ciq-lts94-rocky-release < 10
...
%package     -n %{name_prefix}rocky-repos%{?rltype}
Summary:        %{distro_name} Package Repositories - CIQ LTS %{full_release_version}
License:        %{rpm_license}
...
Obsoletes:      rocky-repos < 10.0

Obsoletes:      ciq-rocky-repos < 10
Conflicts:      ciq-rocky-repos < 10
Obsoletes:      ciq-rocky86-repos < 10
Conflicts:      ciq-rocky86-repos < 10
Obsoletes:      ciq-rocky88-repos < 10
Conflicts:      ciq-rocky88-repos < 10
Obsoletes:      ciq-lts88-rocky-repos < 10
Conflicts:      ciq-lts88-rocky-repos < 10
Obsoletes:      ciq-rocky810-repos < 10
Conflicts:      ciq-rocky810-repos < 10
Obsoletes:      ciq-rocky92-repos < 10
Conflicts:      ciq-rocky92-repos < 10
Obsoletes:      ciq-rocky94-repos < 10
Conflicts:      ciq-rocky94-repos < 10

Conflicts:      %{name_prefix}rocky-cloud-repos%{?rltype}

%description -n %{name_prefix}rocky-repos%{?rltype}
%{distro_name} package repository files for yum/dnf

%package     -n %{name_prefix}rocky-cloud-repos%{?rltype}
Summary:        Cloud Package Repositories - CIQ LTS %{full_release_version}
License:        %{rpm_license}
Provides:       system-repos = %{version}-%{release}
Provides:       %{name_prefix}rocky-repos(%{major}) = %{full_release_version}
Provides:       rocky-repos(%{major}) = %{full_release_version}
...
Conflicts:      ciq-rocky88-repos
Conflicts:      ciq-lts88-rocky-repos
Conflicts:      ciq-rocky810-repos
Conflicts:      ciq-rocky92-repos
Conflicts:      ciq-rocky94-repos

Conflicts:      %{name_prefix}rocky-repos%{?rltype}

%description -n %{name_prefix}rocky-cloud-repos%{?rltype}
...
%package     -n %{name_prefix}rocky-gpg-keys%{?rltype}
Summary:        Rocky and CIQ RPM GPG Keys - CIQ LTS %{full_release_version}
Conflicts:      %{name} < 8.0

Provides:       %{name_prefix}rocky-gpg-keys(%{major}) = %{full_release_version}
Provides:       rocky-gpg-keys(%{major}) = %{full_release_version}
Obsoletes:      rocky-gpg-keys < 10
Conflicts:      rocky-gpg-keys

Obsoletes:      ciq-lts810-rocky-gpg-keys < 10
Conflicts:      ciq-lts810-rocky-gpg-keys < 10
Obsoletes:      ciq-lts94-rocky-gpg-keys < 10
Conflicts:      ciq-lts94-rocky-gpg-keys < 10

%description -n %{name_prefix}rocky-gpg-keys%{?rltype}
...
install -p -m 0644 %{SOURCE1250} %{buildroot}%{_sysconfdir}/yum.repos.d/
...
%files -n %{name_prefix}rocky-repos%{?rltype}
%license docs/LICENSE
...
%files -n %{name_prefix}rocky-gpg-keys%{?rltype}
...
%posttrans -n %{name_prefix}rocky-cloud-repos%{?rltype}
# only run on install, not upgrade
if [ "$1" = "1" ]; then
    if [ ! -f %{_sysconfdir}/dnf/vars/product ]; then
        ln -s %{_sysconfdir}/dnf/vars/product.cloud %{_sysconfdir}/dnf/vars/product
    fi
fi

%preun -n %{name_prefix}rocky-cloud-repos%{?rltype}
...
* Thu Jun 12 2025 Trinity Quirk <tquirk@ciq.com> - 9.6-2.3
- Convert to CIQ LTS96

* Fri May 16 2025 Louis Abel <label@rockylinux.org> - 9.6-1.3
- Update IMA certs
